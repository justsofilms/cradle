...
Database.prototype.fti = function (path, options, callback) {
    ...
    path = path.split('/');
    path = ['_fti', 'local', this.name, '_design', path[0], path[1]].map(encodeURIComponent).join('/');
...
Database.prototype.compact = function (design) {
    this.query({
        method: 'POST',
        path: '/_compact' + (typeof(design) === 'string' ? '/' + encodeURIComponent(design) : ''),
        headers: {
            'Content-Type': 'application/json'
        }
    }, Args.last(arguments));
...
Database.prototype.list = function (path, options) {
    ...
    this._getOrPostView(
        ['_design', path[0], '_list', path[1], path[2]].map(encodeURIComponent).join('/'),
        options,
        callback
    );
};
