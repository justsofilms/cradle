...
Database.prototype.getAttachment = function (id, attachmentName, callback) {
    ...
    return this.connection.rawRequest({
        method: 'GET', 
        path: '/' + [this.name, encodeURIComponent(id), attachmentName].join('/'),
        encoding: null
    }, callback);
};
...
Database.prototype.removeAttachment = function (doc, attachmentName, callback) {
    ...
    this.query({
        method: 'DELETE',
        path: [encodeURIComponent(id), attachmentName].join('/'),
        query: { rev: rev }
    }, callback);
};
...
Database.prototype.saveAttachment = function (doc, attachment, callback) {
    ...
    options.method = 'PUT';
    options.path = '/' + [this.name, encodeURIComponent(id), attachmentName].join('/');
    options.headers = {
